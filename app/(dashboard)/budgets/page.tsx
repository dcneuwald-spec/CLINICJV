...
export default function BudgetsPage() {
  const totalApproved = MOCK_BUDGETS.filter(b => b.status === 'approved').reduce((s, b) => s + b.finalAmount, 0)
  const totalOpen = MOCK_BUDGETS.filter(b => b.status === 'sent' || b.status === 'followup').reduce((s, b) => s + b.finalAmount, 0)
  const conversionRate = Math.round((MOCK_BUDGETS.filter(b => b.status === 'approved').length / MOCK_BUDGETS.length) * 100)
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="card p-4">
            <p className="text-xl font-bold text-slate-900">{MOCK_BUDGETS.length}</p>
            <p className="text-xs text-slate-500 mt-0.5">Total de orçamentos</p>
          </div>
          <div className="card p-4">
            <p className="text-xl font-bold text-emerald-600">{formatCurrency(totalApproved)}</p>
            <p className="text-xs text-slate-500 mt-0.5">Aprovados</p>
          </div>
          <div className="card p-4">
              ...
              <div key={budget.id} className="card p-5">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{budget.patientName} · {formatDate(budget.createdAt)}</p>
                  </div>
                  <span className={cn('badge text-xs flex items-center gap-1', cfg.color)}>
                    {cfg.icon}{getBudgetStatusLabel(budget.status)}
                  </span>
                </div>
                ...
                {(budget.status === 'sent' || budget.status === 'followup') && (
                  <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                    <button className="btn-secondary text-xs flex-1 justify-center">Follow Up</button>
                    <button className="btn text-xs flex-1 justify-center bg-emerald-600 text-white hover:bg-emerald-700">Aprovar</button>
                    <button className="btn-secondary text-xs text-red-600 border-red-100 hover:bg-red-50">Reprovar</button>
                  </div>
                )}
              </div>
            )
